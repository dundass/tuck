...
(function (tw, manifest) {
    ...
    var sounds = {
        ...
        'main': new Howl({src: [manifest.musicMain.src], loop: true, volume: 0.5}),
        'roadside': new Howl({src: [manifest.musicRoadside.src], loop: true, volume: 0.5}),
        'village': new Howl({src: [manifest.musicVillage.src], loop: true, volume: 0.5}),
        'city': new Howl({src: [manifest.musicCity.src], loop: true, volume: 0.5}),
    ...
    tw.tuck.methods = {
        spawnNewDenizen: function () {
            ...
            worldState.denizensActive[denizenData.name] = {
        ...
        startDenizenLoop: function () {
            ...
            setInterval(function () {
                ...
                var denizenNames = Object.keys(worldState.denizensActive);

                for(var i = 0; i < denizenNames.length; i++) {

                    var denizenName = denizenNames[i];

                    var denizen = worldState.denizensActive[denizenName];

                    if(denizenName !== worldState.customer) {
                        worldState.denizensActive[denizenName].x += worldState.denizensActive[denizenName].approachesFrom === 'left' ? 2 : -2;
                    }
                    ...
                    if(denizen.x > 100 || denizen.x < -10) {
                        ...
                        worldState.denizensActive[denizenName] = null;  // maybe ?
                        delete worldState.denizensActive[denizenName];
                        continue;
                    }
                    ...
                    if(worldState.customer === null && scope.sufficientReputation(denizenName) && !uiState.choosingPerks
                        ...
                        worldState.denizensActive[denizenName].x = customerX;
        ...
        loadArea: function (name) {
            ...
            if(name !== oldLocation) {
                this.newDay();
                var oldArea = this.getArea(oldLocation);
                if(oldArea.backgroundAudioId) this.stopSound(oldArea.backgroundAudioId);
            }
        },
        newDay: function () {
        ...
        randomQuestion: function (denizenName) {
            ...
            if(roll && denizen.story && !player.stats.completedStories.includes(denizen.story)) {
                ...
                // just do gold for now. mvp vibes
                if(question.requires) worldState.customerBuys.value = question.requires.gold;
        ...
        answerQuestion: function (e) {
            ...
            if(worldState.customerQuestionType === 'story') {
                ...
                // if end of story chain

                if(nextStage === undefined) {
                    var cantPay = question.requires && question.requires.gold && answer === 'no' && !this.canAfford(question.requires.gold);
                    if(question.recurs || cantPay) {
                        // don't advance if this story stage recurs
                        player.stats.activeStories[activeStoryIdx].stage--;
                        var responsesLength = player.stats.activeStories[activeStoryIdx].responses.length;
                        player.stats.activeStories[activeStoryIdx].responses.splice(responsesLength - 1, 1);
                    }
                    else {
                        // story's over :(
            ...
            // run the yes() or no() func if exists
            if(question[answer]) question[answer](player, worldState, tw.tuck.methods);

            // if resYes/resNo are funcs, run them to get the response
            ...
            // set the customer's response text for ui
        ...
        removeDenizenFromPool: function (name) {
            ...
            var denizenNames = Object.keys(worldState.denizensActive);
            for(var i = 0; i < denizenNames.length; i++) {
                if(worldState.denizensActive[denizenNames[i]].denizenData.name === name) {
                    worldState.denizensActive[denizenNames[i]].returnToPool = false;
                    return;
                }
            }
        },
        returnDenizensToPool: function () {
            var denizenNames = Object.keys(worldState.denizensActive);
            for(var i = 0; i < denizenNames.length; i++) {
                var denizen = worldState.denizensActive[denizenNames[i]];
                if(denizen.returnToPool) {
                    this.addDenizenToPool(denizen.denizenData.name);
                }
                delete worldState.denizensActive[denizenNames[i]];
            }
        },
        ...
        // meta - stages and saves
        toStage: function (stageName) {
            this.stage = stageName;
        },
        toSavegames: function () {
            this.toStage('savegames');
            this.loadSaves();
        },
        newGame: function () {
            this.saveUID = Date.now();  // make an arbitrary UID for saveGame later
            this.toStage('main');
            this.loadArea('roadside');
            this.startDenizenLoop();
        },
        saveGame: function () {
            var existingSaves = localStorage.tucksave ? JSON.parse(localStorage.tucksave) : {};
            var theDate = new Date();
            existingSaves[this.saveUID] = {
                player: player,
                worldState: worldState,
                timestamp: Date.now(),  // for sorting by most recent
                uid: this.saveUID,  // just in case we need it i guess ?
                date: theDate.getDay() + '/' + theDate.getMonth() + '/' + theDate.getFullYear() + ' - ' + theDate.getHours() + ':' + theDate.getMinutes()
            };
            var saveObjectString = JSON.stringify(existingSaves, null, 2);     // TODO - JSON.stringify() doesn't know how to deal with Infinity .......
            localStorage.setItem('tucksave', saveObjectString);
        ...
        loadSaves: function () {
            if(localStorage.tucksave) {
                this.existingSaves = JSON.parse(localStorage.tucksave);
            }
            else {
