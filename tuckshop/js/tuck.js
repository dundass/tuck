...
(function (tw) {
    ...
    // TODO - bubbles + maybe a bubble that denizens say after walking away post-serve

    // TODO - game reacts to ur perk choices by eg presenting ppl asking for herbalist items (die roll when item pick + check items in area)
    // activate perk choice when pass that rep level (but fall below don't remove) // max / min / respec

    // TODO - WHAT IF --- tapping on the person to try and bring them over, if fail checks, they throw it back at ya - poss even choose an item to offer them then decide whether they like - learn about what people want

    // diff people turning up and offering quests / diff speech based on day/night
    ...
    // avatar creator - not mvp
    // hi score & achievements - not mvp

    // higher reputation = more likely to call by name ?

    // denizen interaction - denizens have friends/enemies and compete with / get annoyed by their rivals
    ...
    var sounds = {
        ...
        'coins': new Howl({src: ['audio/coins.mp3']}),
        'close-shop': new Howl({src: ['audio/close-shop.mp3']}),
        'whoosh': new Howl({src: ['audio/whoosh.mp3']}),
        'rep-up': new Howl({src: ['audio/rep-ping-good.mp3']}),
        'rep-down': new Howl({src: ['audio/rep-ping-bad.mp3']}),
        'forest': new Howl({src: ['audio/forest.mp3'], loop: true, volume: 0.5}),
        'stamp': new Howl({src: ['audio/stamp.mp3'], volume: 0.33}),
        'cock': new Howl({src: ['audio/cock.mp3']}),
        'shop-bell': new Howl({src: ['audio/shop-bell.mp3']})
    };
    ...
    tw.tuck.methods = {
        spawnDenizenElement: function () { // TODO - consider moving to root Vue instance via Object assign (cuz of dom)

            setTimeout(this.spawnDenizenElement, (3 + Math.random() * 3) * 1000 * (1 / worldState.dynamics.denizenFrequency));

            if(worldState.endOfDay) return;

            var denizenObject = this.randomDenizen();

            if(typeof denizenObject === 'undefined') {
            ...
            el.style.backgroundImage = 'url(' + denizenObject.image + ')';

            el.setAttribute('data-name', denizenObject.name);
        ...
        denizenLoop: function () {

            var scope = this;

            setTimeout(this.spawnDenizenElement, 3000);
            ...
            setInterval(function () {
                ...
                if(worldState.time > 120 && worldState.customer === null) {
                    ...
                    return;
                } else if((worldState.time < 120 && worldState.customer !== null) || uiState.choosingPerks) {
                    worldState.time += 0.0333;    // maybe increase ?
                } else {
                    worldState.time += 0.1;
                }

                var containerStyle = getComputedStyle(document.getElementById('wrapper'));  // TODO - denizenLoop() should be in root Vue instance
                ...
                for(var i = denizenElements.length - 1; i >= 0; i--) {

                    var style = getComputedStyle(denizenElements[i]);
                    var denizenX = parseFloat(style.left),
                        denizenName = denizenElements[i].getAttribute('data-name');

                    // check if denizen should be deleted
                    if(denizenX > containerWidth || denizenX < -containerWidth / 10) {
                        if(!denizenElements[i].hasAttribute('data-do-not-add')) scope.addDenizenToPool(denizenName);
                        document.getElementById('characterFrame').removeChild(denizenElements[i]);
                        continue;
                    }
                    ...
                    if(tw.Utils.hasClass(denizenElements[i], 'left')) {
                        boundaryLeft = 0.35;
                        boundaryRight = 0.4;
                    } else {
                        boundaryLeft = 0.65;
                    ...
                    if(worldState.customer === null && scope.sufficientReputation(denizenName) && !uiState.choosingPerks
                        && (denizenX > containerWidth * boundaryLeft && denizenX < containerWidth * boundaryRight)) {

                        var denizen = scope.getDenizen(denizenName);

                        if(denizen.story && !player.stats.completedStories.includes(denizen.story)) {
                            var storyStage = scope.getStoryStage(denizen.story);
                            if(storyStage.denizen && storyStage.denizen !== denizenName) return;
                            else if(!storyStage.denizen && scope.questions[denizen.story].denizen !== denizenName) return;
                        } else if(!denizen.asks && player.stats.completedStories.includes(denizen.story)) {
                            return;
                        }

                        // denizenElements[i].style.left = (containerWidth / 2) + 'px';
                        denizenElements[i].style.left = '46%';
                        scope.serveDenizen(denizenName);
                        tw.Utils.addClass(denizenElements[i], 'customer');
                        setTimeout(function () {
                            tw.Utils.addClass(document.getElementById('worldFrame'), 'blurred');
                        }, 1000);
        ...
        spawnGlitzyBit: function (name, startCallback, endCallback) {
        ...
        tapToHide: function () {
            ...
            custElement.style.left = custElement.className.includes('left') ? '120%' : '-20%';
            tw.Utils.removeClass(custElement, 'customer');
        ...
        selectRestockItem: function (e) {
            if(e.target.tagName === 'SPAN' && tw.Utils.hasClass(e.target, 'restock-item')) {
        ...
        loadArea: function (name) {
            ...
            // TODO - iterate elements and add to old denizen pool
        ...
        newDay: function () {
            ...
            if(Math.random() < this.getPerkEffect('Scrounger')) {
                ...
                this.addItemToInventory(itemsInArea[tw.Utils.randomInt(0, itemsInArea)].name);
            }
            ...
            if(roll && denizen.story && !player.stats.completedStories.includes(denizen.story)) {
                ...
                if(questionType === 'purchase') {
                    ...
                    if(denizen.buys) {
                        ...
                        // approach 3 - only ask for things in other areas once you've visited them
                        var scope = this;
                        ...
                        // TODO - incorporate a couple of items from nearby areas too ?
                    ...
                    // or like this ? keeps all 'special' questions in the same location
                    // TODO - probably not long term solution - move this to init prop of question object
                }
        ...
        buyItem: function (item, amount, vendor) {
            ...
            if(this.getPerkEffect('Smooth Talker') && Math.random() < amount / 100) {
                // var itemsInArea = this.getItemsInArea();
        ...
        modifyCurrency: function (amount) {
            ...
            if(amount > 0) {
                for(var i = 0; i < Math.max(1, (amount/10)|0); i++) {
                    setTimeout(function () {
                        this.spawnGlitzyBit('glitz-coin',null, function () {
                ...
                for(var i = 0; i < Math.max(1, Math.abs(amount/10)|0); i++) {
                    setTimeout(function () {
                        this.spawnGlitzyBit('glitz-coin-down', function (glitz) {
        ...
        modifyReputation: function (amount, area) {
            ...
            if(amount > 0) {
                ...
                for(var i = 0; i < Math.max(1, (amount/10)|0); i++) {
                    setTimeout(function () {
                        this.spawnGlitzyBit('glitz-star',null, function () {
                            this.playSound('rep-up');
                        }.bind(this));
                    }.bind(this), i * 40);
                }
                ...
                for(var i = 0; i < Math.max(1, (amount/10)|0); i++) {
                    setTimeout(function () {
                        this.spawnGlitzyBit('glitz-star-down', function (glitz) {
        ...
        // removeUpgrade ? probs not
        hireDenizen: function (name) {  // not mvp
        ...
        removeDenizenFromPool: function (name) {
            // used to splice from denizenPool, but now flags them for not re-adding
            var denizens = document.getElementsByClassName('denizen');
            for(var i = 0; i < denizens.length; i++) {
                if(denizens[i].getAttribute('data-name') === name) {
                    denizens[i].setAttribute('data-do-not-add', 'position absolute');
                    return;
                }
            }
            // var idx = worldState.denizenPool.indexOf(worldState.denizenPool.filter(function (denizen) {
            //     return denizen.name === name;
            // })[0]);
            // return worldState.denizenPool.splice(idx, 1)[0];
        },
        ...
        saveGame: function () {
            ...
            // saveDenizenPool() ? if so, fix to loop thru denizen elements too
